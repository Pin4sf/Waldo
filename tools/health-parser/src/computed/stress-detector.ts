 ...
 */
import type { DailyHealthData, HRRecord, WorkoutRecord } from '../types/health.js';
import type { Baselines } from '../types/crs.js';
import type { StressEvent, DailyStressSummary } from '../types/stress.js';
import { STRESS_WEIGHTS, getStressSeverity } from '../types/stress.js';
import { getTimeOfDayRatio } from '../types/crs.js';
import { computeBaselines } from './baseline-calculator.js';

/** Minimum sustained duration for a stress event (minutes) */
const MIN_DURATION_MINUTES = 10;
/** Cooldown between alerts (minutes) */
const ALERT_COOLDOWN_MINUTES = 120;
/** Max alerts per day */
const MAX_ALERTS_PER_DAY = 3;
/** Window size for HR analysis (minutes) */
...
/** Check if a timestamp falls within any workout period */
function isDuringWorkout(timestamp: Date, workouts: WorkoutRecord[]): boolean {
  return workouts.some(
    w => timestamp >= w.startDate && timestamp <= w.endDate,
  );
}

/** Check if readings indicate low physical activity (sedentary) */
function getActivityScore(readings: HRRecord[]): number {
  ...
  if (activeCount > sedentaryCount) return 1.0; // High activity
  if (sedentaryCount > activeCount * 2) return 0.1; // Very sedentary
  return 0.5; // Mixed
}

/** Compute HR elevation score (0-1) relative to baseline */
...
/** Compute HRV drop score (0-1) relative to time-of-day-adjusted baseline */
function computeHrvDropScore(
  ...
  // Get HRV readings within this window
  ...
  // Normalize each reading by time-of-day, then compare to baseline
...
/** Compute duration score (0-1) — sustained stress is more concerning */
...
/** Slide windows over HR data to detect stress events */
function detectStressWindows(
  day: DailyHealthData,
  baselines: Baselines,
): StressEvent[] {
  ...
  // Sliding window approach: group HR readings into windows
  const windows: HRWindow[] = [];
  let windowStart = 0;

  for (let i = 0; i < hrReadings.length; i++) {
    // Advance window start if gap > window size
    while (
      windowStart < i &&
  ...
  // Find windows with elevated HR (potential stress)
  ...
  for (const window of windows) {
    ...
    // Stress signal: elevated HR + low activity + not during workout
  ...
  // Handle ongoing stress at end of data
...
export function detectDailyStress(
  ...
  // Apply cooldown: keep only events that are ≥2h apart
