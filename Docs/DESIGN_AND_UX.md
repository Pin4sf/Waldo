...
> Consolidated from research session. This is the design reference for the OneSync mobile app.
...
The 5-step onboarding + AI interview is comprehensive but long. Consider:
- **Track completion rates per step** -- if users drop at Step 3 (Health Connect permissions), the pre-permission screen needs work
- **Allow partial onboarding** -- let users skip to dashboard after Step 2 and complete messaging/interview later
- **The AI interview is the magic moment** -- but forcing users into Telegram before they see any value from the app is risky. Consider showing a "demo CRS" on the dashboard first (using mock/population data), THEN prompting for messaging channel link

### 2. Dashboard Information Density
The single-scroll dashboard packs a lot. For MVP:
- **Start with just CRS gauge + insight card + sleep summary.** Add metric cards and weekly trend in Phase 2
- Users need to see ONE thing when they open the app: their CRS score and what it means right now
- The insight card is the most important element after the gauge -- it's the agent's voice in the app

### 3. Missing: Empty States
No designs for:
- Dashboard before any wearable data arrives (first 24 hours)
- Dashboard when Health Connect permissions are denied
- Dashboard when messaging channel is not linked
- These are critical -- many users will see these first

### 4. Missing: Error States
No designs for:
- "Watch not syncing" (background sync killed by OEM)
- "Can't reach server" (offline)
- "Wearable disconnected"
- These happen frequently on Android and need clear, actionable UX

### 5. Accessibility Gaps
- Color-only CRS zones (coral/amber/teal) need text labels for colorblind users -- this is partially addressed but should be tested
- Screen reader flow for the SVG gauge needs explicit `accessibilityLabel`
- Tap targets for metric cards should be minimum 44x44pt

### 6. Consider: Widget
A home screen widget showing just the CRS score and a one-line insight could be the highest-engagement surface. Android widgets are relatively straightforward with Expo. This could be a quick Phase 2 win.

### 7. Design System First
Before building screens, define the design system in Gluestack: spacing scale, color tokens, component variants, shadow values. This saves massive time during implementation and ensures consistency.

### 8. Micro-Interactions Matter
The CRS gauge animation, the trend arrow transitions, the pull-to-refresh feedback -- these small interactions are what make a health app feel "alive" vs "stale." Budget time for polish here.
