# OneSync -- Hardware Strategy (Parallel Track)

Last updated: March 7, 2026 | **Status: PHASE 4 — Not relevant until 100+ paying users**

> **NOT an MVP document.** This covers OneBand custom hardware and NFL/sports market — Phase 4+. Do not context-switch to this during MVP build. Revisit when software product has traction.
>
...
### 1. Two Very Different Markets, One Platform
The consumer wellness market (knowledge workers) and the NFL/sports performance market are radically different in:
- Price sensitivity ($10/mo vs $10K+/team/season)
- Data requirements (HRV-based CRS vs full EEG/EMG cognitive mapping)
- Sales cycle (self-serve vs enterprise sales)
- Regulatory burden (wellness app vs potentially FDA-regulated)

This is both a strength (two revenue streams) and a risk (split focus). Recommendation: **Software MVP proves the consumer thesis first.** Hardware track runs in parallel but shouldn't distract from the core product until Phase 3.

### 2. NFL Go-to-Market Is Harder Than It Looks
NFL teams have:
- Exclusive relationships with existing vendors (Catapult, Kinexon, Zebra)
- Extremely conservative equipment approval processes
- Union (NFLPA) involvement in any biometric monitoring
- Legal teams that will scrutinize data ownership

Consider starting with:
- **College football** (less regulation, more open to innovation)
...
The combination of periauricular EEG + temporal EMG + in-ear PPG + tympanic temp + mastoid IMU for cognitive performance scoring during athletic competition appears to be novel. Consider filing a provisional patent on:
- The sensor fusion algorithm (combining EEG/EMG/PPG/temp/IMU into a single cognitive score)
- The accelerometer-gated EEG acquisition method
- The between-plays cognitive readiness assessment protocol

A provisional patent costs ~$1,500 and gives 12 months of protection while validating the approach.

### 4. Missing: Cost Estimates for Hardware
No BOM (Bill of Materials) or manufacturing cost estimates. Even rough estimates would help:
- EEG amplifier chip (ADS1299 or similar): ~$15-25
- BLE SoC (nRF52840 or similar): ~$3-5
- Flex-PCB fabrication: ~$5-15 per unit at scale
- Total BOM for ear module: likely $50-100 in prototype, $20-40 at 1000 units
- Ansh + Mayur should prepare BOM estimates

### 5. Missing: Battery Life Analysis
Target is 4+ hours, but no power budget is calculated. With EEG amplifier + BLE streaming + IMU + PPG + temperature, 50-100mAh may be tight. Need:
- Component power draw estimates
- Duty cycling strategy (which sensors are always-on vs on-demand?)
- Can EEG duty cycle to 50% (only during between-play windows) to save power?

### 6. Consider: Glasses Form Factor for Phase 6+
If Meta Ray-Ban Display is on the software roadmap, a **glasses-mounted** cognitive sensor makes more sense long-term than behind-ear clips. Glasses naturally sit at the temple, cover the periauricular region, and could include eye tracking. This is a 3-5 year horizon but worth noting in the vision.

### 7. Cricket as an Alternative to NFL
For an India-based startup, consider:
- IPL teams have significant budgets and are early tech adopters
- Cricket involves long periods of sustained concentration (batting, bowling spells)
- Cognitive fatigue in test cricket is well-documented but unmeasured
- The BCCI is more accessible than the NFL for an Indian startup
- Same hardware, different sport -- validate locally before going global
